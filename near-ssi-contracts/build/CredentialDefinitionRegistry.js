...
let CredentialDefinitionRegistry = (_dec = NearBindgen({}), _dec2 = call({}), _dec3 = view(), _dec(_class = (_class2 = class CredentialDefinitionRegistry {
  static schema = {
  ...
  registerCredentialDefinition({
    ...
    if (this.credentialDefinitions.get(credDefId) !== null) {
      return panicUtf8(Uint8Array.from([1]));
    }
...
}, _applyDecoratedDescriptor(_class2.prototype, "registerCredentialDefinition", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "registerCredentialDefinition"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "getCredentialDefinition", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "getCredentialDefinition"), _class2.prototype), _class2)) || _class);
function getCredentialDefinition() {
  const _state = CredentialDefinitionRegistry._getState();
  if (!_state && CredentialDefinitionRegistry._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = CredentialDefinitionRegistry._create();
  if (_state) {
    CredentialDefinitionRegistry._reconstruct(_contract, _state);
  }
  const _args = CredentialDefinitionRegistry._getArgs();
  const _result = _contract.getCredentialDefinition(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(CredentialDefinitionRegistry._serialize(_result, true));
}
function registerCredentialDefinition() {
  ...
  CredentialDefinitionRegistry._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(CredentialDefinitionRegistry._serialize(_result, true));
}

export { getCredentialDefinition, registerCredentialDefinition };
