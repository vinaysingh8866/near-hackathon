...
export default function Sidebar({setSidebarWidth,setCurrentPage,currentPage}:SidebarProps ){
    ...
    return(
        <VStack ref={divRef} w={'full'} maxW={'200px'} minH={'calc(100dvh - 100px)'} h={'full'} position={'fixed'} left={0} top={100} bg={'#1B1B1B'} px={'1.5rem'} py={'2.5rem'}>
            <Button onClick={()=>setCurrentPage(currentPages["Ask For Proof"])} px={'1rem'} display={'block'} textAlign={'left'} color={'white'} bg={currentPage==currentPages["Ask For Proof"]?'#3B3B3B':'none'} w={'full'} variant={'plain'} type="button">
            Ask For Proof
            </Button>
